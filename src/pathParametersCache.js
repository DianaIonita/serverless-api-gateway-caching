...
const addPathParametersCacheConfig = (settings, serverless) => {
  for (let endpointSettings of settings.endpointSettings) {
    ...
    for (let cacheKeyParameter of endpointSettings.cacheKeyParameters) {
      method.resource.Properties.Integration.RequestParameters[`integration.${cacheKeyParameter}`] = `method.${cacheKeyParameter}`;
      method.resource.Properties.Integration.CacheKeyParameters.push(`method.${cacheKeyParameter}`);
    }
